...
with st.sidebar:
    ...
    menu = st.radio(
        ...
        ["🏠 หน้าแรก", 
         ...
         "🕵️ อ่านเจ้ามือ", 
         "📊 ทดสอบกลยุทธ์",
         "📓 สมุดบันทึกการซื้อขาย"],  # เตรียมไว้สำหรับเพจใหม่
...
# ---------- เล่นสั้น ----------
    ...
    # แสดงตัวอย่างคร่าวๆ
    with st.expander("ดูตัวอย่างหน้าตา"):
        st.markdown("""
        ในหน้านี้จะมี:
        - ช่องเลือกหุ้น
        - ราคาปัจจุบัน + การเปลี่ยนแปลง
        - Volume, RSI, MACD
        - กราฟแท่งเทียน + เส้น MA
        - กราฟ Volume
...
# ---------- เล่นยาว ----------
    ...
    with st.expander("ดูตัวอย่างหน้าตา"):
        ...
        ในหน้านี้จะมี:
        - เลือกหุ้น
        - เลือกประเภท: Growth / Dividend
        - ข้อมูลย้อนหลัง 10 ปี (EPS, ROE, ปันผล)
        - กราฟ EPS + กำไร
        - กราฟ ROE + D/E
        - ตารางข้อมูลย้อนหลัง
        - วิเคราะห์ปัจจัยพื้นฐาน
        - คะแนน巴菲特 (Buffett Score)
        - คำแนะนำ ซื้อ/รอ/ผ่าน
        """)
...
# ---------- ทดสอบกลยุทธ์ ----------
    ...
    with st.expander("ดูตัวอย่างหน้าตา"):
        ...
        ในหน้านี้จะมี:
        - เลือกหุ้น
        - เลือกกลยุทธ์ (MA, RSI, ฯลฯ)
        - เลือกช่วงเวลา
        - ผลการทดสอบ (กำไร, Sharpe Ratio)
        - กราฟ Equity Curve
        """)
